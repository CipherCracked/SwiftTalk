...
export default function ContactList({ourId, onlinePeople, offlinePeople, selectedUserId,setSelectedUserId, onlineContacts, offlineContacts, contacts, setContacts}){
    ...
    function searchContact(ev){
        ...
        const offlineSearch={};
        const onlineSearch={};
        Object.keys(onlineContacts).forEach(p=>{
            if (searchQuery.toLowerCase()===onlineContacts[p].username.toLowerCase()) onlineSearch[p]=onlineContacts[p];
        })
        setOnlineSearched(onlineSearch);
        Object.keys(offlineContacts).forEach(p=>{
            if (searchQuery.toLowerCase()===offlineContacts[p].username.toLowerCase()) offlineSearch[p]=offlineContacts[p];
        })
                ...
                searchQuery !== '' ? (
                    <>
                        ...
                        {(Object.keys(onlineSearched).length===0 && Object.keys(offlineSearched).length===0) &&
                            <div className="items-center justify-center bold flex flex-grow h-full">No contacts Found</div>
                        }
                    </>
                ) : (
                    <>
                        {Object.keys(onlineContacts).map(userId => (
                            <Contact
                                key={userId}
                                id={userId}
                                selected={userId === selectedUserId}
                        ...
                        {(Object.keys(onlineContacts).length===0 && Object.keys(offlineContacts).length===0)&&
